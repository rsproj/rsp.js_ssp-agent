...
const log = require('debug')('interactor:ssp:client')
...
module.exports = class PM2Client extends EventEmitter {
  constructor (opts) {
    ...
    this.rpc_sock.on('connect', _ => {
      ...
      this.generateMethods(_ => {
        this.sspInterface = new PM2Interface(this.rpc)
        this.emit('ready')
      })
    })
  ...
  remote (method, parameters, cb) {
    ...
    if (typeof this.sspInterface[method] === 'undefined') {
      return cb(new Error('Deprecated or invalid method'))
    }
    this.sspInterface[method](parameters, cb)
  }
