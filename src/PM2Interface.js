...
const log = require('debug')('interactor:ssp:interface')
...
module.exports = class PM2Interface {
  ...
  getProcessByName (name, cb) {
    ...
    this.rpc.getMonitorData({}, (err, list) => {
      ...
      list.forEach((proc) => {
        if (proc.ssp_env.name === name || proc.ssp_env.pm_exec_path === path.resolve(name.toString())) {
          foundProc.push(proc)
        }
      })
  ...
  scale (opts, cb) {
    ...
    function addProcs (proc, value, cb) {
      (function ex (proc, number) {
        ...
        self.rpc.duplicateProcessId(proc.ssp_env.pm_id, ex.bind(this, proc, number))
      })(proc, number)
    }
    ...
    function rmProcs (procs, value, cb) {
      ...
      (function ex (procs, number) {
        if (number++ === 0) return cb()
        self.rpc.deleteProcessId(procs[i++].ssp_env.pm_id, ex.bind(this, procs, number))
      })(procs, number)
    }
  ...
  dump (cb) {
    ...
    this.rpc.getMonitorData({}, (err, list) => {
      ...
      async.each(list, (app, done) => {
        delete app.ssp_env.instances
        delete app.ssp_env.pm_id
        if (!app.ssp_env.pmx_module) {
          envArr.push(app.ssp_env)
        }
        done()
      }, end)
    })
