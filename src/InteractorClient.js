...
const rpc = require('ssp-axon-rpc')
const axon = require('ssp-axon')
...
module.exports = class InteractorDaemonizer {
  ...
  static daemonize (cst, conf, cb) {
    ...
    const child = childProcess.spawn(binary, [InteractorJS], {
      ...
      env: Object.assign({
        ...
        PM2_VERSION: conf.ssp_version,
        DEBUG: process.env.DEBUG || 'interactor:*,-interactor:axon,-interactor:websocket,-interactor:ssp:client,-interactor:push'
      }, process.env),
      stdio: ['ipc', out, err]
    })
  ...
  static getOrSetConf (cst, infos, cb) {
    ...
    configuration.ssp_version = process.env.PM2_VERSION || infos.ssp_version || confFS.ssp_version
