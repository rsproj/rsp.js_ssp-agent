 ...
 * @param {PM2Client} issp ssp daemon client used to listen on bus
...
module.exports = class PushInteractor {
  constructor (opts, issp, transport) {
    ...
    this.opts = opts
    this.log_buffer = {}
    this.processes = new Map() // Key is process name, value is ssp env
  ...
  start () {
    ...
    this._issp.bus.on('*', this._onPM2Event.bind(this))
  }
   ...
   */
  _worker () {
    if (!this._issp.rpc || !this._issp.rpc.getMonitorData)
    ...
    this._issp.rpc.getMonitorData({}, (err, processes) => {
      if (err) {
      ...
      processes.forEach((process) => {
        this.processes.set(process.name, process.ssp_env)
        this.broadcast_logs.set(process.pm_id, process.ssp_env.broadcast_logs == 1 || process.ssp_env.broadcast_logs == 'true') // eslint-disable-line
      })

      processes = processes.filter(process => process.ssp_env._km_monitored !== false)
