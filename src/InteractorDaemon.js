...
const InteractorDaemon = module.exports = class InteractorDaemon {
  ...
  exit (err, cb) {
    ...
    if (this._issp) this._issp.disconnect()
      ...
      getInfos: function (cb) {
        if (self.opts && self.DAEMON_ACTIVE === true) {
          return cb(null, {
            ...
            ssp_home_monitored: cst.PM2_HOME
          })
        } else {
          return cb(null)
        }
  ...
  start (cb) {
    ...
    const verifyEndpointCallback = (err, result) => {
      ...
      setTimeout(() => {
        ...
        this.watchDog.start({
          ssp_binary_path: this.opts.PM2_BINARY_PATH,
          conf: {
            issp: this.getPM2Client()
          }
        })
      }, 30 * 1000)
