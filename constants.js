...
if (process.env.PM2_HOME) {
  ...
  PM2_HOME = path.resolve(process.env.HOME, '.ssp')
} else if (process.env.HOME || process.env.HOMEPATH) {
  PM2_HOME = path.resolve(process.env.HOMEDRIVE, process.env.HOME || process.env.HOMEPATH, '.ssp')
} else {
  PM2_HOME = path.resolve('/etc', '.ssp')
}
...
let cst = {
  ...
  DUMP_FILE_PATH: path.resolve(PM2_HOME, 'dump.ssp'),
